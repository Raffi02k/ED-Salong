...
export function BookingPage() {
  ...
  return (
    <section className="booking-experience wrap section">
      <div className="booking-intro">
        ...
        <p className="lead">
          Välj behandling och en tid som passar dig. Vi tar hand om detaljerna.
        </p>
        <div className="booking-location">
          <span>ED FRISÖR · TROLLHÄTTAN</span>
          <p>{site.address}</p>
        </div>
      </div>
      <div className="booking-flow">
        <div className="booking-panel">
          <nav className="booking-progress" aria-label="Bokningssteg">
            <ol>
              {["Behandling", "Datum & tid", "Ditt val"].map((label, i) => (
                <li
                  key={label}
                  className={
          ...
          <div className="booking-stage" key={step}>
            {step === 1 ? (
              <>
                ...
                <div
                  className="booking-service-list"
                  role="group"
                ...
                >
                  {services.map((s) => (
                    <button
                      key={s.slug}
                      className="booking-service booking-choice"
                    ...
                    >
                      <span>
                        ...
                        <small>
                          {s.duration} ·{" "}
                          {s.confirmed ? "ungefärligt pris" : "demopris"}
                        </small>
                      </span>
                      <b>{s.price} kr</b>
                    </button>
              ...
              <>
                ...
                <p className="note">
                  Exempeltider för att visa flödet. Ingen koppling till
                  salongens kalender.
                </p>
                ...
                >
                  {days.map((day, i) => {
                    ...
                    return (
                      <button
                        key={value}
                        className="booking-day booking-choice"
                ...
                {!validDate ? (
                  <p className="note">
                    Välj ett datum från idag och framåt för att se exempeltider.
                  </p>
                ) : isClosed ? (
                  <div className="booking-closed-notice" role="status">
                    <p>
                      Salongen är stängd på söndagar. Vänligen välj ett annat
                      datum.
                    </p>
                  </div>
                ) : (
                  <div className="booking-times" key={date}>
                    <p className="booking-label">Välj en exempeltid</p>
                    {[
                      { label: "Förmiddag", times: ["10:00", "11:30"] },
                      {
                        label: "Eftermiddag",
                        times: ["13:00", "14:30", "16:00"],
                      },
                    ].map((group) => (
                      <div className="booking-time-group" key={group.label}>
                        ...
                        <div>
                          {group.times.map((t) => (
                            <button
                              key={t}
                              className="booking-choice booking-time"
                              aria-pressed={time === t}
                              onClick={() => setTime(t)}
                            >
                              {t}
                ...
                <button
                  className="button booking-next"
                  disabled={!validDate || !time || isClosed}
                  onClick={() => setStep(3)}
                >
                  Visa ditt val <span>→</span>
                </button>
              </>
            ) : (
              <>
                <button className="booking-back" onClick={() => setStep(2)}>
                  ← Ändra datum eller tid
                </button>
                <h2 ref={heading} tabIndex={-1}>
                  Ditt nästa besök.
                </h2>
                <div className="booking-selection">
                  <p className="eyebrow">Ditt val · demo</p>
                  <h3>
                    {formattedDate} · {time}
                  </h3>
                  <p>
                    {service.title} · {service.duration} ·{" "}
                    {service.confirmed ? "cirka" : "demopris"} {service.price}{" "}
                    kr
                  </p>
                </div>
                <p>
                  <strong>Ingen bokning har gjorts.</strong> Ring ED Frisör för
        ...
        <p className="booking-help">
          Vill du hellre prata med oss? Ring{" "}
